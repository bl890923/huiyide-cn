...
MobileClient.prototype.changeKfHeaderImg = function(url){
    ...
    $(".mobile-content").find(".mobile-content-box-img").attr("src",url)
    var head_url,http_reg = /^(http|https)/;
    if(http_reg.test(url.toLowerCase()) == true){
        head_url = url
    }else{
        head_url = window.location.origin + url;
    }
    window.parent.postMessage('head_url:' + head_url, '*')
}

//客服图标4动态更新
MobileClient.prototype.changeKfIcon4 = function(type){
    //要从哪个dom获取数据 type 判断类型
    var _this = this;
    //msgtips改变
    if(type != "logo"){
        msgTips_num ++;
        $(".mobile-content-box-num").removeClass("display-none");
        $(".mobile-content").find(".mobile-content-box-num").text('新消息'+msgTips_num)
    }
    //文字msg切换部分
    if(type == 'msg'){
        var getdom = $(".pc-service").eq($(".pc-service").length - 1)
        if(getdom.children(".pc-service-right").find("img").length > 0){
            getdom.children(".pc-service-right").find("img").before($("<p class='linshiadd'>[图片]</p>"))
        }
        var msg = getdom.children(".pc-service-right").find('.pc-service-info').text()
        $(".mobile-content").find(".mobile-content-box-msg").text(msg)
        getdom.children(".pc-service-right").find(".linshiadd").remove()
    }
    if(type == 'flie'){
        $(".mobile-content").find(".mobile-content-box-msg").text('[文件]')
    }
    if(type == 'form'){
        $(".mobile-content").find(".mobile-content-box-msg").text('[表单]')
    }
}

//实时翻译
MobileClient.prototype.onTranslateProc = function(msg,id){
    //要从哪个dom获取数据 type 判断类型
    var _this = this;
...
MobileClient.prototype.showConnMsg = function(msg,type,msgid){
    ...
    if(type == "w"){//展示客服消息时，msgid为头像
        ...
        msg_div += '<div class="pc-service service-info" '+real_msgid+'><div class="pc-service-left" id="'+w_msg_id+'"><img width="34px" src="'+header_url+'" alt=""></div><div class="pc-service-right"> <div class="pc-service-info font14"> <div style="overflow:hidden;">'+msg+'</div> </div>'+msg_btns+'</div> </div>';
        $(".mobile-content").find(".mobile-content-box-img").attr("src",header_url)
    }else if(type == "v"){
        var msgid_html = '',msgid_display = '';
        if(arguments[3] != undefined) msgid_display = 'style="display:none"';//建立对话过程中发的消息，不显示消息发送状态
        if(msgid != undefined && msgid != "") msgid_html = '<i class="info-status" '+msgid_display+' id="'+msgid+'"></i>';
...
MobileClient.prototype.getCopyClueBtn = function(msg){
    ...
    var msg_phone_btn = phone_num == '' ? "" : btn_switch_phone > 0 ? '<a class="msg_btn msg_phone_btn" style="margin-top:0.7rem" data_phone ='+phone_num+' href="tel:'+phone_num+'" onclick="mobile_client.copyTag(this,\'phone\')" target="_top">电话咨询</a>' : "";
...
MobileClient.prototype.showSceneForm = function(obj,id,num){
    ...
    $.each(obj,function(x,y){
        // 英文兼容
        var placeholderStr = y.name =='name' ? _this.configs["mobile_lang_json"][81] :  y.name =='phone' ? _this.configs["mobile_lang_json"][83] : y.name =='wechat' ? _this.configs["mobile_lang_json"][18] : y.name =='qq' ? _this.configs["mobile_lang_json"][17] : y.name =='address' ? _this.configs["mobile_lang_json"][82] : y.name =='email' ? _this.configs["mobile_lang_json"][19] : y.name =='card' ? _this.configs["mobile_lang_json"][87] : y.bname;
        if(y.name == 'phone'){
            var check = y.is_check ==  1 ? ' ischeck' : '';
            form_str = '<div class="from_box form_'+y.name+check+'"><input class="from_input input_'+y.name+'" type="text" placeholder="'+placeholderStr+'"/><div class="phone_ver_code" onclick ="mobile_client.formVerCode(this)">'+_this.configs["mobile_lang_json"][85]+'</div></div>';    
        }else{
...
MobileClient.prototype.changMsgStatus = function(msgid,status){
    ...
    if ($("#"+msgid).length>0) {
        if (msgid.indexOf('formid_') > -1) {
            if(status){//访客表单发送成功
                _this.fkMsgCallBack(msgid,'form');
                $('#'+msgid).remove();
...
//展示任务机器人表单
MobileClient.prototype.showTaskList = function(lists){
    var _this = this;

    var list_content = "";
    $.each(lists, function (i, v) {
        var list_str = "";
        list_str = `<div class="form_list_input" onclick="mobile_client.clickTastList(this)">${
            i + 1
...
//展示任务机器人表单
MobileClient.prototype.showTaskForm = function(forms,submit_text,success_text,fail_text,overtime){
    var _this = this;
    var form_text = "";
    var form_input = "";
    $.each(forms, function (i, v) {
        ...
        form_input_str = `<div class="form_input_input" data-val="${v}"><input style="padding-left: 5px" onfocus="mobile_client.inputFocus(this)"></div>`;
        form_text += form_text_str;
        form_input += form_input_str;
    });
    var form_btn = "";
    form_btn = '<div class="form_preview_btn_box"><button class="blue_btn form_preview_btn" onclick="mobile_client.clickTastSubmitForm(this)" data_issubmit="no" data_success="'+ success_text +'" data_fail="'+ fail_text +'">'+submit_text+'</button></div>';
    _this.talkAppend("<div class='form_box'><div class='form_box_bg'><div class='form_box_textbox'>" + form_text + "</div><div class='form_box_inputbox'>" + form_input + "</div></div>"+form_btn+"</div>");
    ...
    setTimeout(function () {
        $(".form_box").each(function(index,ele){
            if($(ele).find('.form_preview_btn').attr('data_fail') == fail_text && $(ele).find('.form_preview_btn').attr('data_issubmit')=='no'){
                var msg_div = '<div class="pc-service"><div class="pc-service-left"><img width="34px" src="'+_this.configs["zsk_zsktb_url"]+'" alt=""></div><div class="pc-service-right"><div class="pc-service-info font14"><div class="robot-info-answer"><div class="answer-text">'+mobile_client.HTMLDecode(fail_text)+'</div></div></div></div></div>';
                _this.talkAppend(msg_div);
            }
        })
...
MobileClient.prototype.closeShow = function(){
    var _this = this;
    if(_this.configs.is_input == 1 && $(".kf-talk").hasClass("kf-talk-box-hide") && _this.configs.is_minchat == '1'){
        $(".talk-inputArea").addClass("close-talk-hide")
        $("#after_close").addClass("display-none")
        minimizeInfo("1")
        minimizeState("1")
    }
    $("#after_close").show();
...
//      $("#"+upload_progress).remove();
    ...
    xhr.open("post","/visitor/uploadImg.php?type=visiter&company_id="+company_id+"&guest_id="+mobile_client.guest_id);
    xhr.send(fd);
}
...
MobileClient.prototype.showHots = function(){
    ...
    if( robot_hots != undefined && robot_hots.length>0){
        ...
        for (var i=0;i<robot_hots.length;i++){
            // 单引号兼容
            var init_question = robot_hots[i]["question"].replace(/'/g,"\\'");
            var init_answer = robot_hots[i]["answer"].replace(/'/g,"\\'");
            _this.params["robot_question"][robot_hots[i]["id"]] = robot_hots[i];
            msg_div += '<li class="" onclick="mobile_client.checkHotQuestion(\''+init_question+'\',\''+init_answer+'\',\''+robot_hots[i]["id"]+'\')">'+(i+1)+'.'+robot_hots[i]["question"]+'</li>';
        }
...
//展示机器人消息
MobileClient.prototype.showRobotMsg = function(msg,type,q_id,list,hots_info){
    var _this = this;
    var msg_div = _this.getNowTime();
    ...
    if(type == "w"){
        ...
        if(list != undefined && list.length>0){//关联问题
            ...
            for (var i=0;i<list.length;i++){
                ...
                //else
                    msg_div += '<li class=" " onclick="mobile_client.checkHotQuestion(\''+list[i]["question"].replace(/\'/g,"\\\'")+'\',\''+list[i]["answer"]+'\',\''+list[i]["id"]+'\')">'+(i+1)+'.'+list[i]["question"]+'</li>';
            }
            msg_div += '</ul>';
        }else if( hots_info != undefined && hots_info != '' ){
            msg_div += '<ul class="question-lists color-blue">';
            for (var i=0;i<hots_info.length;i++){
                //if(_this.configs["is_huawei_robot"] == "1")
                //    msg_div += '<li class=" " onclick="mobile_client.sendRobotMsg(\''+list[i]["question"]+'\')">'+(i+1)+'.'+list[i]["question"]+'</li>';
                //else
                    msg_div += '<li style="cursor: pointer; color: #1F8CEB; line-height: 2.4rem; font-size: 1.2rem; height: 2.4rem; max-width: 30rem; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;" onmouseup="this.style.color=\'#62778C\'" onclick="mobile_client.checkHotQuestion(\''+hots_info[i]["question"].replace(/\'/g,"\\\'")+'\',\''+hots_info[i]["answer"]+'\',\''+hots_info[i]["id"]+'\')">'+(i+1)+'.'+hots_info[i]["question"]+'</li>';
            }
            msg_div += '</ul>';
        }
...
MobileClient.prototype.matchQuesShow = function(msg){
    ...
    for (var i= 0;i<length;i++){
        robotAcqHtml += '<li class="match-question-list" onclick ="mobile_client.sendRobotMsg(\''+msg.rows[i].question.replace(/\'/g,"\\\'")+'\',\''+msg.rows[i].answer+'\')">'+msg.rows[i].question+'</li>';
    }
...
MobileClient.prototype.changModule = function(module){
    ...
    if(module == "reg"){//注册模块
        ...
        if(_this.configs.is_input == 1 && _this.configs.is_minchat == '1' && $(".select-kf").hasClass("select-kf-box-hide")){
            minimizeInfo("1")//foot.html封装的方法 通讯传递高度
            minimizeState("1")
        }else if(_this.configs.is_input == 1 && _this.configs.is_minchat == '1'){
            minimizeInfo("1")//foot.html封装的方法 通讯传递高度
        }
        $(".kf-talk").hide();//隐藏对话显示模块
        ...
        if(module == "showkf"){//对话
            ...
            if (_this.configs["zsk_api_robot_id"] != "" || _this.configs["task_robot_use"] == "1" || _this.configs["zsk_lwswitch"] == "1") {
                $(".to-lword").hide();
            }
        ...
        // $(".kf-talk").show();//显示对话模块
        $(".kf-talk").show();
        if(_this.configs.is_input == 1 && _this.configs.is_minchat == '1' && $(".kf-talk").hasClass("kf-talk-box-hide")){
            minimizeInfo("2")//foot.html封装的方法 通讯传递高度
            minimizeState("1")
        }else{
            minimizeInfo("2")//foot.html封装的方法 通讯传递高度
        }
        ...
        $(".info-input").hide();//隐藏注册模块
        $(".lword").show();//隐藏留言模块
        if(_this.configs.is_input == 1 && _this.configs.is_minchat == '1' && $(".lword").hasClass("lword-box-hide")){
            minimizeInfo("1")//foot.html封装的方法 通讯传递高度
            minimizeState("1")
        }else{
            minimizeInfo("1")//foot.html封装的方法 通讯传递高度
        ...
        var head_title = company_name == "" ? _this.configs["mobile_lang_json"][11] : company_name;
        //处理头部
        $(".header-title").html(head_title).removeClass("right-square-bracket");//修改头部信息
        $(".icon_card_info").hide();//隐藏客服名片
        ...
        var head_title = company_name == "" ? _this.configs["mobile_lang_json"][11] : company_name;
        //处理头部
        $(".header-title").html(head_title);//修改头部信息
        $(".icon_card_info").hide();//隐藏客服名片
        $(".header-more").show();//显示头部右侧加号
        $(".more-wrap .border_none").removeClass("border_none");
 ...
 * @return {[type]}     新字符串
 */
MobileClient.prototype.stringCheck = function( str, len ) {
    var len = len || 30;
    var count = 0;
    var newStr = str;
...
//访客表单单选选择器
MobileClient.prototype.showVisitorForm = function(title,res,mustids,formid,button){
    var _this=this;
    ...
    for(var i=0;i<id_arr.length;i++){
        ...
        var is_must_test = is_must === '' ? '' : '（'+_this.configs["mobile_lang_json"][86]+'）';
        var form_item=_this.configs['visitor_form'][id_arr[i]];
        if(form_item.field_type=='1'){//输入框
            if(form_item.field_name=='name'){
                visitor_form_html+='<div data-id="'+id_arr[i]+'" class="visitor_form_ques nomal'+is_must+'"><input class="visitor_form_input1" data-type="name" type="text" placeholder="'+_this.stringCheck(form_item.name)+is_must_test+'"><p class="tip_error">'+_this.configs["mobile_lang_json"][70]+form_item.name+'</p></div>';//请输入
            }else if(form_item.field_name=='mobile'){
                visitor_form_html+='<div data-id="'+id_arr[i]+'" class="visitor_form_ques nomal'+is_must+'"><input class="visitor_form_input1" data-type="mobile" type="text" placeholder="'+_this.stringCheck(form_item.name)+is_must_test+'"><p class="tip_error">'+_this.configs["mobile_lang_json"][69]+form_item.name+'</p></div>';//请输入有效的
            }else if(form_item.field_name=='weixin'){
                visitor_form_html+='<div data-id="'+id_arr[i]+'" class="visitor_form_ques nomal'+is_must+'"><input class="visitor_form_input1" data-type="wechat" type="text" placeholder="'+_this.stringCheck(form_item.name)+is_must_test+'"><p class="tip_error">'+_this.configs["mobile_lang_json"][69]+form_item.name+'</p></div>';
            }else if(form_item.field_name=='qq'){
                visitor_form_html+='<div data-id="'+id_arr[i]+'" class="visitor_form_ques nomal'+is_must+'"><input class="visitor_form_input1" data-type="qq" type="text" placeholder="'+_this.stringCheck(form_item.name)+is_must_test+'"><p class="tip_error">'+_this.configs["mobile_lang_json"][69]+form_item.name+'</p></div>';
            }else if(form_item.field_name=='email'){
                visitor_form_html+='<div data-id="'+id_arr[i]+'" class="visitor_form_ques nomal'+is_must+'"><input class="visitor_form_input1" data-type="email" type="text" placeholder="'+_this.stringCheck(form_item.name)+is_must_test+'"><p class="tip_error">'+_this.configs["mobile_lang_json"][69]+form_item.name+'</p></div>';
            }else{
                visitor_form_html+='<div data-id="'+id_arr[i]+'" class="visitor_form_ques'+is_must+'"><input class="visitor_form_input1" type="text" placeholder="'+_this.stringCheck(form_item.name)+is_must_test+'"><p class="tip_error">'+_this.configs["mobile_lang_json"][70]+form_item.name+'</p></div>';
            }
        }else{//单选
            var single_item=form_item.option_config;
            single_item=single_item.toString();
            visitor_form_html+='<div data-id="'+id_arr[i]+'" class="visitor_form_ques single_choose'+is_must+'" single_item="'+single_item+'"><input class="visitor_form_input2" type="text" placeholder="'+_this.stringCheck(form_item.name)+is_must_test+'" readonly="readonly"><span class="slide_down_btn"></span><p class="tip_error">'+_this.configs["mobile_lang_json"][71]+form_item.name+'</p></div>';//请选择
        }
    }
    if (typeof(button) == 'undefined' || button == '') button = _this.configs["mobile_lang_json"][66];//提交表单
    visitor_form_html+='<p class="visitor_form_submit">'+button+'</p></div>';//提交表单
    $('#talk_content').append(visitor_form_html);
    _this.changeKfIcon4("form")
    chatAnimate.scroll_bottom();
...
// 播放音效
MobileClient.prototype.playSound = function(){
    var _this=this;
    if (_this.configs['fk_music'] != '') {
        try{
            var myAuto = document.getElementById('audioSound');
            $("#audioSound").attr("src",http_pro+host+"/sound/"+_this.configs['fk_music']+".mp3");
            myAuto.play();
        }catch(e){}
    }
}
...
MobileClient.prototype.setupFilter = function(str){
    var _this = this;
    // 待处理str
    var newStr = str || '';
    // 按钮,行内选项,行外选项
    ...
    if(!!outsideRowBtns || !!outsideRowOpts){
        _this.sendApiForConnTotal('show');
    }
 ...
 * 匹配规则：[[]]
 */
MobileClient.prototype.kwFilter = function(str){
    var _this = this;
    var newStr = str || '';
    ...
    if(matchKw && matchKw.length > 0){
        for(var i = 0; i<matchKw.length; i++){
            ...
            var replaceStr = matchfilterStr.substring(2,matchfilterStr.length-2);
            if (_this.configs["keyword"] != '') replaceStr = _this.configs["keyword"];
 ...
 */
MobileClient.prototype.sendApiForConnTotal = function(type){
    var _this = this;
    $.ajax({
        type: "GET",
        url: "statistical_test.php?type=conn_"+type+"&company_id="+company_id+"&guest_id="+mobile_client.guest_id+"&device=mobile",
        dataType: "json",
 ...
 * 客服微信二维码
 */
 MobileClient.prototype.showServiceQRcode = function(msg,msgid){
    var _this = this;
    ...
    if(arguments[2] != undefined && arguments[2] != '') {
        real_msgid = 'id="'+arguments[2]+'"';
    }
    var str = '<div class="pc-service service-info" '+real_msgid+'><div class="pc-service-left" id="'+w_msg_id+'"><img width="34px" src="'+header_url+'" alt=""></div><div class="pc-service-right"><div class="service-qrcode-box"><div class="msg-box">'+ msgStr +'</div><div class="img-box"><div class="img-border"><img class="qrcode-picture" src="'+ imgUrl +'"><img class="wx-logo" src="../../style/setting/ver07/img/new2017/wx-logo.svg"></div></div></div></div></div>';
